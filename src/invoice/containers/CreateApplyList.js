...
class CreateApplyList extends Component {
	...
	async componentDidMount() {
		...
		//获取meta的信息
		this.props.actions.getMeta().catch(({ errorMsg }) => {
			message.error(errorMsg || '请求出错', 1)
		});
		if (this.props.data.type) {
			this.props.actions.invoiceState(this.props.comp_id, this.props.data.type).catch(({ errorMsg }) => {
				message.error(errorMsg || '请求出错', 1)
			});
		} else {
			this.props.actions.invoiceState(this.props.comp_id, 1).catch(({ errorMsg }) => {
				message.error(errorMsg || '请求出错', 1)
			});
		}
		this.props.actions.getToken().catch(({ errorMsg }) => {
			message.error(errorMsg || '请求出错', 1)
		});
	...
	handleRadioValue(e) {
		...
		switch (radio) {
			...
			case 2:
				...
				this.props.actions.invoiceState(this.props.comp_id, 2).catch(({ errorMsg }) => {
					message.error(errorMsg || '请求出错', 1)
				});
	...
	handleInvoiceTitle = async (value) => {
		if (value) {
			await this.props.actions.getInvoiceTitleRelation(value).catch(({ errorMsg }) => {
				message.error(errorMsg || '请求出错', 1)
			});
	...
	invoiceTitleSelect = async (value) => {
		...
		await this.props.actions.getInvoiceTitleContent(selectedObj.id).catch(({ errorMsg }) => {
			message.error(errorMsg || '请求出错', 1)
		});
			...
			<fieldset style={{ textAlign: 'left' }} className='apply_box'>
				...
				<Form onSubmit={this.handleSubmit.bind(this)} >
					...
					{this.state.upload_contract && this.props.token.contract_scan_file ?
						<FormItem label="上传信息" extra={<div style={{ color: 'red' }}>最多可上传5个附件，单附件不能超过10M，支持格式：zip、rar(压缩包)、pdf</div>} {...formItemLayout} className="formItem">
							{getFieldDecorator('contract_scanning_copy', {
								...
								<WBYUploadFile tok={{ token: this.props.token.contract_scan_file.token, upload_url: this.props.token.upload_uri }} listType='text' multiple={true} size={10} len={5} accept={".zip,.rar,.pdf"} uploadText={'上传合同扫描件'} beforeUpload={beforeUpload}>
								</WBYUploadFile>
							)}
						</FormItem> : null
					}
