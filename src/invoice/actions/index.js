...
export const getApplyMetDada = () => (dispatch) => {
	return api.get('/finance/invoice/application/metadata').then((response) => {
		const { data } = response
		dispatch({
...
export const getSaleList = () => (dispatch) => {
	return api.get('/finance/invoice/application/saleList').then((response) => {
		const { data } = response
		dispatch({
			type: GET_SALELIST,
			payload: {
...
export const getCreateList = () => (dispatch) => {
	return api.get('/finance/invoice/application/createUserList').then((response) => {
		const { data } = response
		dispatch({
			type: GET_CREATELIST,
			payload: {
...
export const getApplyListStat = (values) => (dispatch) => {
	return api.get('/finance/invoice/application/listStat', { params: { ...values } }).then((response) => {
		const { data } = response
		dispatch({
...
//发票列表接口
export const getAvailableInvoiceList = (invoice_application_id, exclude_invoice_ids, page, page_size) => (dispatch) => {
	let ids = JSON.stringify(exclude_invoice_ids)
	return api.get('/finance/invoice/application/availableInvoiceList?invoice_application_id=' + invoice_application_id + '&exclude_invoice_ids=' + ids, page, page_size).then((response) => {
		const { data } = response
		dispatch({
