...
class contract extends React.Component {
	...
	handleSearch = (e) => {
		...
		this.props.form.validateFields((err, fieldsValue) => {
			...
			if (fieldsValue['created_time']) {
				...
				values = {
					...fieldsValue,
					'from_created_time': rangeValue.length > 0 ? rangeValue[0].format('YYYY-MM-DD') : '',
					'to_created_time': rangeValue.length > 0 ? rangeValue[1].format('YYYY-MM-DD') : ''
				};
			} else {
				values = { ...fieldsValue };
			}
	...
	render() {
		...
		let contractPagination = {
			onChange: (current) => {
				this.props.actions.getApplyList({ page_no: current });
			},
