...
const tdSubList = {
    ...
    '需求ID/需求名称/活动名称_for_yuyue': [
        {title: '需求ID', key: '需求ID'},
        {title: '需求名称', key: '需求名称'},
    ],
    '执行完成/结算时间': [
        {title: '执行完成时间', key: '执行完成时间'},
        {title: '结算时间', key: '结算时间'},
    ],
    '结案/开票时间': [
        {title: '结案时间', key: '结案时间'},
...
export const getCreditCol = ( excludeCol = [] ) => {
    const allCol =  [
        {
            title: '订单/活动ID',
            dataIndex: '订单/活动ID',
        ...
        },
        {
            ...
            width: 100,
            render: (_, record) => {
                const isYuyue = record.yiyu;
                const subKey = isYuyue ? '需求ID/需求名称/活动名称' : '需求ID/需求名称/活动名称_for_yuyue';
                return tdRender(tdSubList[subKey], record)
            }
        },
        {
            title: '使用信用金额',
        ...
        },
        {
            ...
            key: '执行完成/结算时间',
            width: 100,
            render: (_, record) => tdRender(tdSubList['执行完成/结算时间'], record)
        },
        {
            title: '结案/开票时间',
            dataIndex: '结案/开票时间',
