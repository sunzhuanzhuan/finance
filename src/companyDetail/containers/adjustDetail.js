...
class AdjustDetail extends Component {
	constructor() {
		super();
		this.state = {
			loading: false,
	...
	queryData = (obj, func) => {
		...
		return this.props.actions.getReadjustPriceAccountBill({ ...obj }).then(() => {
	...
	render() {
		...
		const { companyData, readjustPriceAccountBill: { page, page_size, total, list = [], total_amount, revoke_amount }, companyMetadata: { is_revoke = [], readjust_account_bill_type = [] } } = this.props;
		const paginationObj = {
			onChange: (current) => {
					...
					<Form className='adjust-stat'>
						...
						<FormItem label='类型' className='left-gap'>
							{getFieldDecorator('type', { initialValue: { key: '', label: '全部' } })(
								...
								>
									<Option value={''}>全部</Option>
									{readjust_account_bill_type.map(item => (<Option key={item.id} value={item.id}>{item.display}</Option>))}
								</Select>
							)}
						</FormItem>
...
const mapStateToProps = (state) => ({
	...
	readjustPriceAccountBill: state.companyDetail.readjustPriceAccountBill,
	companyMetadata: state.companyDetail.companyMetadata,
})
const mapDispatchToProps = (dispatch) => ({
	actions: bindActionCreators({ ...companyDetailAction }, dispatch)
})
export default connect(mapStateToProps, mapDispatchToProps)(Form.create()(AdjustDetail));
