...
class Filter extends Component {
	...
	getList = async () => {
		...
		obj.page = 1;
		obj.page_size = 20
		await this.props.actions.getRepartionList({ ...obj }).catch(({ errorMsg }) => {
	...
	handlePage(page) {
		this.setState({
			...
			this.props.actions.getRepartionList({ ...obj }).catch(({ errorMsg }) => {
				message.error(errorMsg || '请求出错', 1)
			})
		})
	...
	componentDidMount() {
		...
		this.props.actions.getRepartionStatus().catch(({ errorMsg }) => {
			message.error(errorMsg || '请求出错', 1)
		});
	...
	async handleQuerySucc() {
		...
		obj.page = 1;
		obj.page_size = 20
		await this.props.actions.getRepartionList({ ...this.state }).catch(({ errorMsg }) => {
			message.error(errorMsg || '请求出错', 1)
		});
