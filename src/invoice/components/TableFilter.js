...
import { Select, Row, Col, Input, Icon, Button, Spin, Modal, DatePicker, Form, message } from 'antd';
...
class tableFilter extends Component {
	...
	handleFilter = async () => {
		...
		tableLoaing(true)
		await getOrderList({ tabType, ...filters, page: 1, id: this.props.id, ...values }).catch(({ errorMsg }) => {
			message.warning(errorMsg, 1);
		})
		tableLoaing(false)
	}
